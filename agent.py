...
- Формат buyer_address: "РФ, г. <город>, ул. <улица>, д. <номер>, корп. <корпус>, кв. <номер>"
  ...
  Пример: "РФ, г. Екатеринбург, ул. Краснолесья, д. 114, корп. 2, кв. 233"
...
- ОБЯЗАТЕЛЬНО сверяй оба написания между собой.
- Если оба написания совпадают — используй это значение.
- Если написания РАЗЛИЧАЮТСЯ (например из-за плохого качества скана) — НЕ выбирай одно из них
  наугад. Вместо этого укажи оба варианта пользователю и спроси, какой верный, например:
  "⚠️ Серия/номер паспорта читаются по-разному в разных местах: XXXX YYYYYY и XXXX ZZZZZZ —
  уточните правильный номер".
...
- В ТПО и таможенной декларации ФИО продавца указано в ТРЁХ ОТДЕЛЬНЫХ колонках/строках:
  ФАМИЛИЯ, ИМЯ, ОТЧЕСТВО (например отдельные подписи "(фамилия)", "(имя)", "(отчество)").
- seller_name ОБЯЗАН включать ВСЕ ТРИ части: "Фамилия Имя Отчество" полностью, без сокращений.
- Перед тем как вписать seller_name, перечитай все три колонки по отдельности и убедись,
  что ни одна не пропущена (особенно среднее слово — имя, его легко упустить).
