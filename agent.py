...
- На основном развороте паспорта серия и номер указаны печатным шрифтом ВЕРТИКАЛЬНО на
  ...
  используй именно эту вертикальную печатную надпись как источник серии/номера.
- В НИЖНЕЙ части страницы с фотографией некоторых паспортов есть машиночитаемая зона (MRZ) —
  одна или две длинные строки из ЗАГЛАВНЫХ ЛАТИНСКИХ БУКВ, цифр и множества символов "<".
  Если такая зона присутствует — ИГНОРИРУЙ её полностью при определении серии/номера паспорта
  (цифры там закодированы в ином формате и НЕ являются серией/номером, несмотря на внешнее
  сходство). MRZ может отсутствовать на старых паспортах — это нормально, ищи вертикальную
  печатную надпись независимо от наличия MRZ.
