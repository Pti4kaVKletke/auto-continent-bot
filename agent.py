...
- На основном развороте паспорта серия и номер указаны ДВАЖДЫ печатным шрифтом:
  1) ВЕРТИКАЛЬНО на правом поле страницы крупными цифрами, читается СВЕРХУ ВНИЗ,
     формат "XX XX XXXXXX" (например "65 17 525507" → серия "6517", номер "525507").
  2) Та же информация повторяется ВЕРТИКАЛЬНО на правом поле страницы с фото (нижняя
     половина разворота) — это ТА ЖЕ серия/номер, что и на верхней половине, должны совпадать.
- БЕРИ СЕРИЮ И НОМЕР ТОЛЬКО ИЗ ЭТИХ ВЕРТИКАЛЬНЫХ ПЕЧАТНЫХ НАДПИСЕЙ.
- ЗАПРЕЩЕНО вычислять или извлекать серию/номер из машиночитаемой зоны (MRZ — две строки
  с латинскими буквами/цифрами и символами "<" в самом низу страницы с фото). Цифры в MRZ
  закодированы в формате, который НЕ совпадает напрямую с печатной серией/номером (это
  легко спутать, но формула ненадёжна и даёт неверный результат). MRZ можно использовать
  только для сверки ФИО, даты рождения и пола, но НЕ для серии/номера паспорта.
...
- ОБЯЗАТЕЛЬНО сверяй вертикальную надпись с верхней и нижней половины разворота между собой —
  они должны совпадать (это одна и та же серия/номер, продублированная на обеих половинах).
- Если надписи на двух половинах различаются, или вертикальная надпись нечитаема —
  НЕ выбирай значение наугад. Укажи пользователю, что нашёл, и спроси подтверждение, например:
  "⚠️ Серия/номер паспорта читаются как 'XXXX YYYYYY' (вертикальная надпись на полях) —
  подтвердите, что это верно, либо уточните серию/номер".
