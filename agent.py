...
- На основном развороте паспорта серия и номер указаны печатным шрифтом ВЕРТИКАЛЬНО на
  правом поле страницы, крупными цифрами, читается СВЕРХУ ВНИЗ, формат "XX XX XXXXXX"
  (например "65 17 525507" → серия "6517", номер "525507"). Эта надпись повторяется на
  верхней и нижней половине разворота (на нижней — рядом с фотографией) и должна совпадать.
- В САМОМ НИЗУ страницы с фотографией есть ДВЕ СТРОКИ МАШИНОЧИТАЕМОЙ ЗОНЫ (MRZ) —
  длинные строки из ЗАГЛАВНЫХ ЛАТИНСКИХ БУКВ, цифр и множества символов "<<<<<<".
  Пример MRZ:
    PNRUSVYSOTINA<<OLES8<ANDREEVNA<<<<<<<<<<<<<<<
    6515255076RUS8406309F<<<<<<7171009660012<80
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО брать серию/номер из MRZ-строк (тех, что содержат "<<<" и
  латинские буквы). Цифры там закодированы иначе и НЕ являются серией/номером паспорта,
  несмотря на внешнее сходство. ПЕРЕД тем как назвать серию/номер, проверь: если источник
  цифр — строка с символами "<", это MRZ и брать её ЗАПРЕЩЕНО, нужно искать ВЕРТИКАЛЬНУЮ
  надпись на правом поле страницы (отдельно от MRZ, без символов "<", цифры идут по одной
  в строке сверху вниз).
  Конкретно для примера выше: MRZ начинается с "6515255076" — это НЕ серия/номер.
  Настоящая серия/номер — отдельная вертикальная надпись на полях страницы, в этом примере
  это "65 17 525507" (серия "6517", номер "525507").
- MRZ можно использовать только для сверки ФИО, даты рождения и пола, но НЕ для серии/номера.
- НЕ ПУТАЙ серию/номер паспорта с "Кодом подразделения" (формат "XXX-XXX", например "660-103")
  — это орган, выдавший паспорт (поле passport_code), указан рядом с датой выдачи в верхней
  половине разворота, и это ДРУГОЙ номер.
- ОБЯЗАТЕЛЬНО сверяй вертикальную надпись с верхней и нижней половины разворота между собой —
  они должны совпадать.
- Если вертикальная надпись нечитаема или две половины не совпадают — НЕ выбирай значение
  наугад и НЕ подставляй цифры из MRZ как "запасной вариант". Вместо этого явно напиши
  пользователю: "⚠️ Не удалось чётко прочитать серию/номер паспорта (вертикальная надпись
  на полях) — уточните серию и номер вручную".
