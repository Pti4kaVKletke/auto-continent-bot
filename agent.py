...
# ── Системный промпт ─────────────────────────────────────────────────────
# Текст промпта лежит в system_prompt.md рядом с ботом (вынесен из f-строки
# 25.09.2026: любая фигурная скобка в тексте правила раньше роняла бота
# целиком). Подставляется только сегодняшняя дата — по метке <<СЕГОДНЯ>>,
# обычной заменой строки, поэтому скобки в тексте теперь просто текст.
# Инструкции, компании и банковские профили дописываются в
# _build_system_prompt кодом, как и раньше.
# Файла нет или метки нет → бот падает сразу при старте, с понятной ошибкой,
# а не работает молча без правил.
PROMPT_TODAY_MARK = "<<СЕГОДНЯ>>"
_PROMPT_PATH = Path(__file__).resolve().parent / "system_prompt.md"


def _load_system_prompt() -> str:
    if not _PROMPT_PATH.exists():
        raise RuntimeError(f"Не найден файл системного промпта: {_PROMPT_PATH}")
    text = _PROMPT_PATH.read_text(encoding="utf-8")
    # Правка на Windows может принести \r\n и перевод строки в конце файла.
    text = text.replace("\r\n", "\n").rstrip("\n")
    if PROMPT_TODAY_MARK not in text:
        raise RuntimeError(f"В {_PROMPT_PATH.name} нет метки {PROMPT_TODAY_MARK} для даты")
    return text
...
class DocumentAgent:
    ...
    def _build_system_prompt(self) -> str:
        ...
        base = SYSTEM_PROMPT_TEMPLATE.replace(PROMPT_TODAY_MARK, today_str)
    ...
    async def extract_salon_card(self, filepath: str = None, filename: str = None,
        ...
        props["brand"]["description"] = ("торговое название салона, если оно указано отдельно от юридического "
                                         "(напр. «Казах Авто»); иначе пусто")
