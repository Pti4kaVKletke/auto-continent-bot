...
cash_amount      — НЕОБЯЗАТЕЛЬНОЕ. Фактическая сумма наличных в долларах, реально
                   выданная продавцу. Становится известна только ПОСЛЕ конвертации,
                   при создании сделки её нет — оставляй пустым.
cash_amount_words — НЕОБЯЗАТЕЛЬНОЕ, считается само по cash_amount.
...
1a. ДВА КУРСА И ДВЕ СУММЫ В ВАЛЮТЕ — не путай их:
   ...
   - «Фактический курс» — курс, по которому банк реально конвертировал деньги.
     ...
     сделки его НЕ спрашивай.
   - cash_amount — фактически выданная продавцу сумма в валюте. Если её не вписали
     руками, считается сама: car_price / Фактический курс.
            ...
            },
            {
                ...
                "input_schema": {
                    ...
                    "properties": {
                        ...
                        "updates": {
                            "type": "object",
                            "description": "Словарь полей для обновления: {имя_поля: новое_значение}. Имена полей — те же что в data при create_contract, плюс 'Статус', 'Комментарий', 'Фактический курс', 'Дата поступления', 'Дата расчёта', 'Номер ДКП', 'Дата ДКП'.",
                        },
                    },
