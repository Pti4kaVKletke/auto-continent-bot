...
def _settlement_base(deal: dict) -> float:
    """Рублёвая база Поручения — то, что фактически уходит Получателю.

    Закрывающие документы (расписка, акт, отчёт) описывают не договор, а
    фактическое движение денег: сколько Принципал реально перечислил, сколько
    из этого составило вознаграждение Агента и сколько получил Продавец.
    Поэтому база считается от поступивших денег, а не от цены ДКП.

    Порядок источников:
      1. «Сумма выдана (USD)» × «Фактический курс» — если колонка заполнена,
         документы по сделке уже выдавались; база восстанавливается из них,
         и повторная генерация даёт те же цифры, что в подписанном экземпляре
         (в том числе если Принципал потом доплатил).
      2. Полная оплата — база равна цене ДКП (car_price). Поведение, которое
         было до появления этой функции; все старые сделки идут сюда.
      3. Неполная оплата — база = Получено / (1 + Комиссия% / 100). Комиссия
         берётся процентом от того, что реально прошло через Агента:
         база + база × % = Получено.
    ...
    if paid_currency and rate_fact:
        ...
        # иначе полностью оплаченные сделки поехали бы на копейки.
        if abs(restored - price) <= 1.0:
            return price
        return restored

    total     = _calc_total_amount(deal)
    received  = sum(p["amount"] for p in _parse_payments(deal.get("Платежи", "")))

    if not received or received >= total - 0.01:
        return price
...
def _settlement_date_missing_text(contract_number: str) -> str:
    """
    Просьба указать дату расчёта с продавцом.

    Раньше она подставлялась датой последнего платежа — но день, когда деньги
    пришли на счёт, и день, когда наличные переданы продавцу, обычно разные.
    Этой датой подписаны расписка, акт и отчёт, поэтому угадывать её нельзя.
    """
    return "\n".join([
        f"⚠️ По сделке *{contract_number}* не указана дата расчёта с продавцом.",
        "",
        "Это день, когда наличные переданы получателю — им датируются расписка,",
        "акт и отчёт агента.",
        "Пришлите дату — например «дата расчёта 27.08.2026» — и я впишу её",
        "в журнал и сформирую документ.",
    ])


def _fact_rate_missing_text(contract_number: str) -> str:
    """
    Просьба ввести фактический курс — без него расписка, акт и отчёт неполны.

    Фактический курс появляется только после конвертации, поэтому при создании
    сделки его не спрашивают. К моменту расчёта с продавцом он уже известен из
    банковской выписки, и именно он стоит в расписке, акте и отчёте.
    """
    return "\n".join([
        f"⚠️ По сделке *{contract_number}* не заполнен фактический курс конвертации.",
        "",
        "Он появляется после конвертации и отличается от курса в Поручении.",
        "Пришлите его — например «фактический курс 11,85» — и я впишу его в журнал",
        "и сформирую документ. Если сумма наличных в долларах отличается от расчётной,",
        "укажите и её.",
    ])
...
Отвечай на русском языке. Будь краток и по делу."""
        ...
        if bank_profile_names:
            ...
            base += (
                "\n"
                "ПРАВИЛА РАБОТЫ С БАНКОВСКИМИ ПРОФИЛЯМИ:\n"
                "1. Когда нужен ВЫБОР профиля (при создании сделки, при смене реквизитов "
                "в существующей сделке, при вопросе «какие профили есть» / «покажи реквизиты» / "
                "«какие сохранённые банки», при фразе «используй профиль» без имени) — "
                "ВСЕГДА вызывай инструмент request_bank_choice. Он покажет пользователю "
                "тапабельные кнопки: сохранённые профили + «Новые реквизиты». "
                "НЕ пересказывай список профилей текстом, НЕ спрашивай «какой из профилей "
                "вам нужен?» словами — это плохой UX на мобильном.\n"
                "2. Когда пользователь уже назвал профиль по имени («используй ВТБ-Бакай», "
                "«возьми Тинькофф-Бакай») — не спрашивай ничего, сразу подставляй данные "
                "из соответствующего профиля выше.\n"
                "3. После того как пользователь тапнул кнопку с профилем (его выбор придёт "
                "как обычное сообщение с именем профиля) — подставь данные этого профиля в "
                "поля сделки. Если пользователь тапнул «Новые реквизиты» — спроси реквизиты "
                "текстом и после ввода предложи сохранить через save_bank_profile с названием "
                "вида «Банк - Имя получателя» (например «Альфа Банк - Бакай»).\n"
                "4. Если пользователь просит удалить профиль («удали профиль X», «убери из "
                "памяти N») — вызывай delete_bank_profile. Если N — порядковый номер, "
                "сопоставь с последним показанным списком (порядок соответствует "
                "тому что выше в этом промпте)."
            )
        else:
            base += (
                "Сохранённых профилей пока нет.\n\n"
                "ПРАВИЛА:\n"
                "1. Когда нужны банковские реквизиты (создание сделки, смена реквизитов, "
                "вопрос «какие профили есть») — вызывай request_bank_choice. Он покажет "
                "кнопку «Новые реквизиты» (других вариантов пока нет). НЕ спрашивай "
                "«какие реквизиты использовать?» текстом без кнопок.\n"
                "2. После того как пользователь укажет реквизиты текстом, предложи "
                "сохранить их через save_bank_profile с названием вида «Банк - Имя получателя»."
            )
    ...
    def _get_tools(self) -> list:
        return [
            ...
            },
            {
                ...
                "input_schema": {
                    ...
                    "properties": {
                        ...
                        "updates": {
                            "type": "object",
                            "description": "Словарь полей для обновления: {имя_поля: новое_значение}. Имена полей — те же что в data при create_contract, плюс 'Статус', 'Комментарий', 'Фактический курс', 'Дата поступления', 'Дата расчёта', 'Номер ДКП', 'Дата ДКП'. Колонку 'Сумма выдана (USD)' бот заполняет сам — не трогай её.",
                        },
                    },
            ...
            {
                "name": "delete_bank_profile",
                "description": (
                    "Удалить сохранённый банковский профиль из памяти по названию. "
                    "Используй когда пользователь говорит «удали профиль X», «убери из памяти "
                    "реквизиты Y», «забудь профиль Z», «удали из памяти N» (где N — номер из "
                    "последнего показанного списка профилей). "
                    "Название должно точно совпадать с существующим профилем. Если пользователь "
                    "назвал порядковый номер («удали 3») — сопоставь с последним показанным "
                    "списком в этом диалоге и подставь настоящее имя. Если сомневаешься какой "
                    "именно профиль имеется в виду — сначала уточни у пользователя, не удаляй "
                    "наугад. Операция необратима."
                ),
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "name": {
                            "type": "string",
                            "description": "Точное название профиля, например «ВТБ-Бакай»",
                        },
                    },
                    "required": ["name"],
                },
            },
            {
                ...
                "description": (
                    ...
                    "сохранённые профили + кнопка «Новые реквизиты». "
                    ...
                    "(«поменяй реквизиты в NNN», «замени банк»); "
                    "(4) пользователь говорит «используй профиль» / «возьми из памяти» без "
                    "указания конкретного имени. "
                    "НЕ вызывай если пользователь уже назвал конкретный профиль по имени "
                    "(«используй ВТБ-Бакай») — сразу подставляй его данные. "
                    "НЕ повторяй список профилей текстом — даже если он есть в системном "
                    "промпте, всё равно вызывай этот инструмент, чтобы пользователь мог "
                    "тапнуть кнопку а не набирать имя вручную."
                ),
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "message": {
                            "type": "string",
                            "description": "Короткий текст-подпись над кнопками, например «Какие реквизиты использовать для этой сделки?» или «Выбери профиль»",
                        },
                    },
                    "required": ["message"],
                },
    ...
    async def _execute_tool(self, tool_name: str, tool_input: dict) -> dict:
        ...
        if tool_name == "create_contract":
            ...
            # Тип покупателя пишем в журнал всегда, одним из двух значений.
            # Пусто → "физ.лицо" (как и считает doc_builder по умолчанию),
            # любые варианты «ип / индивидуальный предприниматель» → "ИП".
            ...
            if not name:
                return {"error": "⚠️ Не указано название профиля"}

            existing = memory.list_bank_profiles()
            if not existing:
                return {"error": "⚠️ В памяти нет ни одного банковского профиля"}

            # Точное совпадение → мягкое совпадение (без учёта регистра/пробелов/дефисов)
            if name not in existing:
                norm = lambda s: s.lower().replace(" ", "").replace("-", "").replace("«", "").replace("»", "")
                target = norm(name)
                match = next((p for p in existing if norm(p) == target), None)
                if not match:
                    return {"error": (
                        f"⚠️ Профиль «{name}» не найден. "
                        f"Доступные: {', '.join(existing)}. "
                        "Уточни точное название."
                    )}
                name = match

            memory.delete_bank_profile(name)
            return {"message": f"🗑 Профиль «{name}» удалён из памяти."}
            ...
            # Автоматическая корректировка статуса после удаления платежа:
            #   была «завершена», остаток > 0
            #     • ещё остались платежи → «ждём доплату»
            #     • платежей нет         → «активна»
            #   была «ждём доплату», платежей больше нет → «активна»
            ...
            if current_status == "завершена" and remainder > 0.01:
                if payments:
                    ...
                    status_msg = "Статус возвращён на «активна» (платежей больше нет)"
            elif current_status == "ждём доплату" and not payments:
                updates["Статус"] = "активна"
                status_msg = "Статус возвращён на «активна» (платежей больше нет)"
    ...
    async def add_payment_impl(
        ...
        # Дата поступления средств от принципала — она же дата платежа, второй
        # раз спрашивать её незачем. Пишем только в пустую колонку: при доплате
        # значение уже стоит, а ручную правку затирать нельзя. При нескольких
        # платежах остаётся дата первого — если для отчёта нужна другая,
        # поправьте колонку вручную.
        if not str(deal.get("Дата поступления") or "").strip():
            updates["Дата поступления"] = date
    ...
    async def _ensure_paid_amount(self, contract_number: str, deal: dict) -> dict:
        ...
        Считается как база Поручения / «Фактический курс» с округлением до
        цента. База — _settlement_base: цена ДКП при полной оплате, иначе
        фактически поступившее за вычетом вознаграждения Агента.
        Пишем В ЖУРНАЛ и только потом подставляем в расписку, акт и отчёт —
        не наоборот. Иначе при повторной генерации документов число
        пересчитается и разойдётся с уже подписанной распиской.

        Колонка «Сумма нал. (USD)» здесь ни при чём: она посчитана по курсу
        поручения и уходит только в поручение.
        """
        ...
        value = f"{paid:.2f}".replace(".", ",")
        deal = dict(deal)
        deal["Сумма выдана (USD)"] = value
        ok = await self.sheets.update_deal(contract_number, {"Сумма выдана (USD)": value})
        if ok:
            logger.info(f"Сделка {contract_number}: сумма выдана {value} записана в журнал")
        else:
    ...
    async def build_act_impl(self, contract_number: str) -> dict:
        ...
        if not payments:
            return {"error": (
                ...
                "Акт формируется по фактически поступившим средствам."
            )}
        ...
        try:
            ...
            docx_path = await self.builder.build_act(
                ...
                jitter_spec=spec, settlement_price=_settlement_base(deal),
            )
    ...
    async def build_report_impl(self, contract_number: str) -> dict:
        ...
        if not payments:
            return {"error": (
                f"⚠️ По сделке *{contract_number}* нет зарегистрированных платежей. "
                "Отчёт агента формируется по фактически поступившим средствам."
            )}
    ...
    async def build_receipt_impl(self, contract_number: str) -> dict:
        ...
        if not payments:
            return {"error": (
                f"⚠️ По сделке *{contract_number}* нет зарегистрированных платежей. "
                "Расписка формируется по фактически поступившим средствам."
            )}
