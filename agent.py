...
- На развороте паспорта есть ДВА РАЗНЫХ вида печатных надписей с цифрами — их легко спутать
  при повороте изображения, поэтому различай их по СОДЕРЖИМОМУ, а не по положению на странице:

  А) ВЕРТИКАЛЬНАЯ НАДПИСЬ С СЕРИЕЙ/НОМЕРОМ — это ТОЛЬКО ЦИФРЫ (без букв вообще), сгруппированные
     по 1-2 цифры в строке, читаются одна группа под другой, формат при чтении сверху-вниз
     даёт "XX XX XXXXXX" (например "65", "17", "525507" → серия "6517", номер "525507").
     Эта надпись повторяется ДВАЖДЫ на развороте (на верхней половине и на половине с фото) —
     оба раза должны давать ОДИНАКОВЫЙ результат "XX XX XXXXXX". ЭТО и есть искомые
     серия и номер.

  Б) МАШИНОЧИТАЕМАЯ ЗОНА (MRZ) — это строка/строки, содержащие ЗАГЛАВНЫЕ ЛАТИНСКИЕ БУКВЫ
     вперемешку с цифрами и множеством символов "<" (например "RUS", "PN", буквы имени
     транслитом, много "<<<<"). MRZ есть только ОДИН РАЗ на развороте (не повторяется).
     КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО брать серию/номер из MRZ — даже если в начале MRZ видна
     последовательность из 10 цифр похожая на "XXXXXXXXXX", это НЕ серия/номер.

  Признак для различения: если в строке есть ЛАТИНСКИЕ БУКВЫ или символы "<" — это MRZ,
  игнорируй её. Если строка/группа состоит ТОЛЬКО из цифр и повторяется дважды на
  развороте — это серия/номер (вариант А).
  MRZ может отсутствовать на старых паспортах — в этом случае просто ищи вариант А,
  который повторяется дважды.
- НЕ ПУТАЙ серию/номер паспорта с "Кодом подразделения" (формат "XXX-XXX", например "660-103",
  с дефисом и всего 6 цифр) — это орган, выдавший паспорт (поле passport_code), указан рядом
  с датой выдачи, и встречается на развороте ОДИН раз (не повторяется), в отличие от
  серии/номера.
- ОБЯЗАТЕЛЬНО сверяй вариант А с верхней и нижней половины разворота между собой —
  они должны совпадать.
- Если вертикальная надпись нечитаема или две половины не совпадают — НЕ выбирай значение
  наугад. Вместо этого явно напиши пользователю: "⚠️ Не удалось чётко прочитать серию/номер
  паспорта (вертикальная надпись на полях) — уточните серию и номер вручную".

ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА перед тем как зафиксировать passport_series/passport_number:
1. Прочитай вертикальную надпись (вариант А) на ВЕРХНЕЙ половине разворота — запомни цифры.
2. Прочитай вертикальную надпись (вариант А) на ПОЛОВИНЕ С ФОТО — запомни цифры.
3. Сравни эти два прочтения между собой посимвольно.
4. В своём ответе пользователю ВСЕГДА покажи оба прочтения и результат сравнения, например:
   "Серия/номер: верх разворота — 6517 525507, низ разворота (с фото) — 6517 525507 ✅ совпадает"
   или при несовпадении:
   "⚠️ Верх разворота читается как 6517 525507, низ — 6514 525507 — не совпадает,
   уточните серию/номер вручную".
5. Используй итоговое значение в данных ТОЛЬКО если оба прочтения совпали. Если совпали
   случайно с цифрами из MRZ — это нормально и ожидаемо для некоторых паспортов, главное
   что ты сделал два независимых прочтения вертикали и они совпали между собой.
6. Если изображение нечёткое и ты не уверен хотя бы в одной цифре с первой попытки —
   попробуй ВТОРУЮ ПОПЫТКУ: мысленно поверни область с вертикальной надписью на 90°
   против часовой стрелки, чтобы цифры серии/номера расположились горизонтально в ряд
   (как обычный текст), и прочитай их в этой ориентации. Иногда это помогает точнее
   разобрать мелкие цифры. Повтори шаги 1-4 с этой ориентацией.
7. Если и после второй попытки нет уверенности хотя бы в одной цифре — прямо попроси
   пользователя прислать отдельное приближенное (zoom) фото правого края страницы с
   серией/номером, не угадывая отдельные цифры.
