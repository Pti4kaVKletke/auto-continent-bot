...
- На основном развороте паспорта серия и номер указаны ДВАЖДЫ: горизонтально вверху страницы
  (мелким шрифтом, формат "XX XX XXXXXX") и ВЕРТИКАЛЬНО на правом поле страницы (крупными цифрами,
  формат "XX XX XXXXXX" читается сверху вниз).
- НЕ ПУТАЙ серию/номер паспорта с "Кодом подразделения" (формат "XXX-XXX", например "660-103") —
  это РАЗНЫЕ номера. Код подразделения относится к ОРГАНУ, выдавшему паспорт (поле
  passport_code), и стоит рядом с датой выдачи в верхней половине разворота. Серия/номер
  паспорта гражданина — это ВЕРТИКАЛЬНАЯ надпись крупными цифрами на правом поле страницы
  (например "65 14 830071") и/или мелкая горизонтальная надпись в самом верху страницы.
- ОБЯЗАТЕЛЬНО сверяй вертикальную и горизонтальную надписи между собой.
- Если оба написания совпадают — используй это значение.
- Если написания РАЗЛИЧАЮТСЯ, или ты видишь только ОДНУ надпись и не уверен — НЕ выбирай
  значение наугад и НЕ путай его с кодом подразделения. Вместо этого укажи пользователю,
  что нашёл, и спроси подтверждение, например:
