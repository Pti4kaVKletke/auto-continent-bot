...
ALLOW_EMPTY_PLACEHOLDERS = {
    ...
    "{{КОММЕНТАРИЙ}}",
    "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА1}}", "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА2}}", "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА3}}",
...
# удаляется из документа целиком. Нужен для строк, которые печатаются лишь
# в одном варианте (напр. отступ под «ИП ФИО» — у физлица его быть не должно,
# иначе получится двойной пустой интервал). 25.09.2026.
...
PLACEHOLDER_LABELS = {
    ...
    "{{ПОКУПАТЕЛЬ_ПОЛНЫЕ_ДАННЫЕ}}": "полные данные покупателя (ФИО/ИП/ООО, паспорт, ИНН)",
    "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА1}}": "банковские реквизиты покупателя (р/сч, опционально)",
    "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА2}}": "банковские реквизиты покупателя (банк, опционально)",
    "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА3}}": "банковские реквизиты покупателя (корр.счёт, опционально)",
    "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА4}}": "банковские реквизиты покупателя (БИК, опционально)",
    "{{ПОКУПАТЕЛЬ_ПОДПИСЬ_СТРОКА}}": "строка подписи покупателя в блоке реквизитов (ФИО+дата рождения либо «ИП ФИО»)",
    "{{ПОКУПАТЕЛЬ_ИНН_СТРОКА}}":     "строка ИНН покупателя в блоке реквизитов (только для ИП, опционально)",
...
class DocumentBuilder:
    ...
    def _resolve_buyer_legal_fields(self, data: dict) -> dict:
        ...
        в блоке «ПРИНЦИПАЛ» договора) и переопределяет {{ПОКУПАТЕЛЬ_ИНИЦИАЛЫ}}
        (короткая подпись) в зависимости от типа покупателя.

        buyer_type: "физ.лицо" (по умолчанию, для старых сделок без этого
        поля) или "ИП". Паспортные данные и адрес для ИП — те же самые (ИП
        остаётся физлицом с паспортом), меняется только вступительная фраза:
        вместо «Гражданин(ка) РФ» — «Индивидуальный предприниматель ...,
        действующего на основании Свидетельства ИНН ...». Формулировка взята
        из сделки 240726001, которую Илья довёл вручную (15.09.2026).

        buyer_bank_details — одно опциональное поле (для любого buyer_type, но
        на практике нужно в основном ИП — п. 5.4 Договора про возврат средств
        на реквизиты Принципала), свободный текст: Илья вносит в ОДНУ ячейку
        журнала как ему удобно (одной строкой или в несколько строк через
        Alt+Enter). Если пусто — блок в документе просто не печатается
        (пустые абзацы уже заложены в шаблоне).
        """
        data = dict(data)
        n = self._normalize
        ...
        passp_by      = n(data.get("passport_issued_by", ""))
        passp_date    = data.get("passport_issued_date", "")
        passp_code    = data.get("passport_code", "")
        address       = n(data.get("buyer_address", ""))

        passport_part = (
            f"паспорт: серия {passp_series} № {passp_number}, "
            f"выдан: {passp_by}, {passp_date}, "
            f"код подразделения: {passp_code}, "
            f"зарегистрирован(а): {address}"
        )
        ...
        if is_ip:
            ...
            # (buyer_initials из журнала); здесь только добавляем префикс «ИП».
            base_initials = n(data.get("buyer_initials", ""))
            data["buyer_initials"] = f"ИП {base_initials}".strip()
            # Блок «Реквизиты и подписи сторон» (см. ниже {{ПОКУПАТЕЛЬ_ПОДПИСЬ_СТРОКА}}/
            # {{ПОКУПАТЕЛЬ_ИНН_СТРОКА}}) — для ИП вместо «ФИО ДАТА_РОЖДЕНИЯ» печатаем
            # «ИП ФИО» и добавляем строку с ИНН (было пусто — Илья заметил 15.09.2026:
            ...
            # Пустая строка между «ИП ФИО» и строкой ИНН (как в образце 240726001).
            data["buyer_signature_ip_gap"] = ""
        else:
            data["buyer_full_details"] = (
            ...
            # физ.лицо — {{ПОКУПАТЕЛЬ_ИНИЦИАЛЫ}} остаётся как есть (не трогаем)
            data["buyer_signature_name_line"] = f"{name} {birth_date}".strip()
            data["buyer_signature_inn_line"] = ""
            # У физлица строки-отступа нет: абзац удаляется, остаётся прежний
            # вид «ФИО дата» → пустая строка (пустой ИНН) → паспорт.
            data["buyer_signature_ip_gap"] = DROP_PARAGRAPH

        # Банковские реквизиты покупателя (собственный р/сч) — печатаются в
        # блоке «ПРИНЦИПАЛ» договора (АГ/АГ-прямой), сразу под адресом.
        # Одно свободнотекстовое поле журнала (buyer_bank_details) — Илья сам
        # решает, как его заполнить: одной строкой или в несколько (Alt+Enter
        # в Google Sheets записывает перенос строки как символ \n внутри
        # ячейки). В шаблоне под это заложено 3 строки-плейсхолдера
        # ({{ПОКУПАТЕЛЬ_БАНК_СТРОКА1..3}}), поэтому раскладываем построчно:
        # первые 2 строки — как есть, всё, что сверх — склеивается в 3-ю
        # (запятыми), чтобы не терять данные, если строк оказалось больше.
        # Пусто — все три плейсхолдера пустые, блок не печатается (они в
        # ALLOW_EMPTY_PLACEHOLDERS). НЕ пропускаем через n()/_normalize —
        # это точные реквизиты (как ИНН/БИК/VIN), а не ФИО или адрес, и
        # _normalize ломает регистр в «АО «Альфа-Банк»» (не распознаёт
        # заглавные буквы внутри кавычек-ёлочек как отдельные слова).
        raw_bank = str(data.get("buyer_bank_details") or "").strip()
        bank_lines = [ln.strip() for ln in raw_bank.splitlines() if ln.strip()]

        data["buyer_bank_line1"] = bank_lines[0] if len(bank_lines) >= 1 else ""
        data["buyer_bank_line2"] = bank_lines[1] if len(bank_lines) >= 2 else ""
        data["buyer_bank_line3"] = bank_lines[2] if len(bank_lines) >= 3 else ""
        # 25.09.2026: 4-я строка (БИК отдельной строкой, как в ручном образце
        # 240726001); всё сверх 4 строк склеивается в неё через запятую.
        data["buyer_bank_line4"] = ", ".join(bank_lines[3:]) if len(bank_lines) >= 4 else ""
    ...
    async def _fill_template(self, template_path, data, number, date, output_name,
        ...
        replacements = {
            ...
            # Не через n() — это точные банковские реквизиты (как БИК/ИНН),
            # регистр в них уже собран верно в _resolve_buyer_legal_fields().
            "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА1}}":  data.get("buyer_bank_line1", ""),
            "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА2}}":  data.get("buyer_bank_line2", ""),
            "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА3}}":  data.get("buyer_bank_line3", ""),
            "{{ПОКУПАТЕЛЬ_БАНК_СТРОКА4}}":  data.get("buyer_bank_line4", ""),
            # Блок «Реквизиты и подписи сторон» (ПРИНЦИПАЛ): строка ФИО(+дата
            # рождения)/«ИП ФИО» и строка ИНН — уже собраны с верным регистром
            # в _resolve_buyer_legal_fields(), через n() не пропускаем.
            "{{ПОКУПАТЕЛЬ_ПОДПИСЬ_СТРОКА}}": data.get("buyer_signature_name_line", ""),
            "{{ПОКУПАТЕЛЬ_ИНН_СТРОКА}}":     data.get("buyer_signature_inn_line", ""),
            "{{ПОКУПАТЕЛЬ_ОТСТУП_ИП}}":      data.get("buyer_signature_ip_gap", DROP_PARAGRAPH),
        ...
        # ── Удаляем абзацы-маркеры DROP_PARAGRAPH (см. константу) ──────
        for p_elem in list(doc.element.body.iter(f"{{{W}}}p")):
            p_text = "".join(t.text or "" for t in p_elem.iter(f"{{{W}}}t"))
            if DROP_PARAGRAPH in p_text and not p_text.replace(DROP_PARAGRAPH, "").strip():
