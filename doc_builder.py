...
class DocumentBuilder:
    ...
    def _normalize(value: str) -> str:
        ...
        Аббревиатуры (МКК, СОМ, УФМС и т.д.) всегда остаются в верхнем регистре.
        """
        ...
        # Аббревиатуры — всегда верхний регистр
        # МКК/СОМ — органы выдачи ID карт КР (районные комитеты)
        # УФМС/МВД/ОМВД/МФЦ — российские органы выдачи паспортов
        # ОАО/ООО/ОсОО/АО/ЗАО/ПАО — организационно-правовые формы
        # РФ/КР/СНГ — страны/союзы
        UPPER_WORDS = {
            # Органы КР
            ...
            # Орг. формы
            "ОАО", "ООО", "ОсОО", "АО", "ЗАО", "ПАО", "ГУП", "МУП", "НКО", "ИП",
            # Страны и союзы
            "РФ", "КР", "СНГ", "СССР", "США", "ЕС",
            # Прочие частые аббревиатуры
            "ИНН", "КПП", "БИК", "ОГРН", "СНИЛС", "ВИН", "VIN",
            "ТПО", "ДКП", "АГ",
        }
        ...
        for i, word in enumerate(words):
            # Убираем знаки препинания для проверки по словарю
            clean = word.strip('.,;:()«»"\'')
            if clean.upper() in UPPER_WORDS:
                # Аббревиатура — оставляем верхний регистр, но знаки препинания сохраняем
                result.append(word.upper())
            elif any(c.isdigit() for c in word):
                # Слова с цифрами — capitalize только первую букву
