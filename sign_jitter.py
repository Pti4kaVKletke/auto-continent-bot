...
#
# По вертикали у печати запас невелик: в шаблоне v2 под её нижним краем
# остаётся около 5 мм до края страницы 1, поэтому вниз она ходит на 3 мм,
# вверх — на 5. Двигаете печать в шаблоне — проверьте этот зазор заново.
#
...
def _fit(tpl_blob: bytes, new_blob: bytes, cx: int, cy: int):
    """Рамка и поправка к позиции для картинки из набора.

    Высота росчерка приравнивается к высоте росчерка из шаблона: подписи
