...
#
# По горизонтали: печать от -14 до +6 мм, подпись ±5 мм — подпись должна
# остаться на своей линии и не налезать на фамилию рядом.
#
...
PROFILES = {
    ...
    "stamp": {
        # Коридор несимметричный: живой оттиск ложится на подпись или чуть
        # левее, вправо так далеко не убегает. При ±20 мм в отчёте агента
        # печать уходила на 47–86 мм от края листа — целиком правее подписи,
        # под фамилию и в сторону колонки принципала.
        "dx_mm": (-14.0, 6.0), "dy_mm": (-5.0, 6.0),
        # Живой оттиск ставится в НАСТОЯЩИЙ размер печати, а не в размер
        # рисованного факсимиле из шаблона (оно рисовалось 38,8 мм).
        # 40,4 — это 40 мм плюс 1% на замеренную усадку: печать документа
        # и обрезка слабых пикселей по краю оттиска дают около процента.
        # Мерили по трём распечатанным документам, где рядом стоит живая
        # печать: напечатанная выходила меньше на 0,4–1,6%.
        "ink_mm": 40.4,
        # ±10°: оттиск от руки ровно почти никогда не ложится, но и на 30°
        # его не заваливают. Вдобавок сканы в наборе уже несут собственный
        # наклон (от -17° до +4°, СКО 4,5°) — код добавляет поверх него,
        # поэтому свой коридор держим скромным. На габариты поворот не
        # влияет: оттиск круглый, вращается только кольцо текста.
        "rot_deg": 10.0,
        "scale": 0.0,
        "alpha": (0.78, 0.97),
        "blur": (0.0, 0.6),
