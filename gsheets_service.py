...
COLUMNS = [
    ...
    "Статус",
    "buyer_type",   # "физ.лицо" (по умолчанию) или "ИП" — вставлена Ильёй
                    # физически 15.09.2026 ПЕРЕД buyer_name («Тип Покупателя»,
                    # J). ИСПРАВЛЕНО 15.09.2026 вечером: первая версия правки
                    # ошибочно ставила эту колонку ПОСЛЕ buyer_name (J=name,
                    # K=type) — это не совпадало с реальной таблицей (скрин
                    # подтвердил J=«Тип Покупателя» пустой, K=«ФИО покупателя»
                    # с данными) и роняло check_deal/generate_docs, читавших
                    # buyer_name из пустой колонки J. Порядок ниже — по факту.
    "buyer_name",   # K
    "passport_series",
    "passport_number",
    "buyer_birth_date",
    "buyer_inn",    # ИНН покупателя, только если buyer_type = "ИП" — вставлена
                    # Ильёй физически 15.09.2026 между buyer_birth_date и
                    # buyer_address («ИНН Покупателя»). ПОЗИЦИЯ ПОКА НЕ
                    # ПЕРЕПРОВЕРЕНА скриншотом после находки бага выше — раз
                    # buyer_type/buyer_name были перепутаны местами, buyer_inn
                    # мог быть вставлен по той же ошибке на одну колонку раньше
                    # (перед buyer_birth_date, а не после). Не менять вслепую —
                    # сначала свежий скриншот колонок M–Q.
    "buyer_address",
    "buyer_initials",
    "passport_issued_by",
    ...
    "corr_bank_bic",
    "corr_bank_acc",
    "buyer_bank_details",  # банковские реквизиты ПОКУПАТЕЛЯ (не путать с бан-
                           # ковскими полями выше — те про счёт компании) —
                           # свободный текст, п. 5.4 Договора про возврат
                           # средств, нужен в основном у ИП. Вставлена Ильёй
                           # физически 15.09.2026 в конец блока «Финансы»,
                           # сразу после corr_bank_acc, перед «Комментарий»
    ...
    # ── Блок «ТЕХНИЧЕСКОЕ» ────────────────────────────────────────────
    # Служебные поля, которые пишет и читает только бот. Человеку править
    # их не нужно; новые технические колонки добавляются сюда, в конец.
    "Штамп и подпись",  # что выпало подписи и печати при ПЕРВОЙ сборке
                        # документа: файл росчерка, сдвиги, угол, плотность
                        # чернил (формат — sign_jitter.format_spec).
                        # При пересборке значения берутся отсюда, поэтому
                        # повторный акт выглядит ровно как выданный ранее.
                        # Стереть ячейку = разрешить новый розыгрыш.
]
